...
void LoadData(BPTree<double>& index, double* keys, int num_keys, string keys_file_path){
...
void runLookupBenchmarkOneBatchAlex(BPTree<double>& index, int num_keys, double* keys){
...
int main(int argc, char* argv[]) {
    ...
    auto keys = new double[10000];

    std::string keys_file_type = "binary";
    if (keys_file_type == "binary") {
        std::cout<<"Loading binary data "<<std::endl;
        load_binary_data(keys, 180000, keys_file_path);
    } else if (keys_file_type == "text") {
        load_text_data(keys, 180000, keys_file_path);
    } else {
    ...
    LoadData(index, keys, 180000, keys_file_path);
    runLookupBenchmarkOneBatchAlex(index, 180000, keys);
